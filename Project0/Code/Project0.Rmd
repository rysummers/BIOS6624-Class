...
```{r}
# extract observations actually used by model (excludes NAs in model)
df_used <- model.frame(agrmnt.model2)

# beta coefficients
betas <- fixef(agrmnt.model2)

fit <- predict(agrmnt.model2, re.form = NA) 
se <- predict(agrmnt.model2, re.form = NA, se.fit = TRUE)$se.fit 
lower <- fit - 1.96 * se 
upper <- fit + 1.96 * se
...
# model equation for plotting
eq_label <- sprintf(
  "italic(y) == %.2f %s %.2f*italic(x)",
  betas[1],
  ifelse(betas[2] < 0, "-", "+"),
  abs(betas[2]))
...
ggplot() +
  geom_point(
    data = spit_df,
    aes(x = mem_min_since_wk, y = bk_min_since_wk),
    alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, 
              linetype = 2, color = "firebrick3") +

  geom_ribbon(
    data = pred_df,
    aes(x = mem_min_since_wk, ymin = lower, ymax = upper),
    fill = "gray35", alpha = 0.25) +
  
  geom_line(
    data = pred_df %>% dplyr::arrange(mem_min_since_wk),
    aes(x = mem_min_since_wk, y = fit),
  ...
  labs(
    title = "Booklet vs Cap Collection Time",
    x = "Cap minutes since wake",
    y = "Booklet minutes since wake") +
  theme_minimal(base_size = 10)
...
tbl1 <- spit_df2 %>%
  ...
  tbl_summary(
    ...
    label = list(
      time_diff_min ~ "Booklet vs Cap Time (min)",
  ...
  modify_footnote_header(
    footnote = "Table includes all subject from cohort",
...
spit_df_filt <- spit_df %>% 
  ...
  mutate(
    ...
    t30_cap = pmin(mem_min_since_wk, 30),
    t_after30_cap = pmax(0, mem_min_since_wk - 30))
```
...
tidy(cortisol.model, conf.int=T, exponentiate=T)
...
```{r}
# build cortisol model
cortisol.model2 <- lmer(log_cortisol ~ t30_cap + t_after30_cap +
                   (1|subjectid),
                 data=spit_df_filt)

summary(cortisol.model2)
tidy(cortisol.model2, conf.int=T, exponentiate=T)

```


```{r}
# quick check of number of patients and 
lme4::getME(cortisol.model, "flist") %>%  lapply(nlevels)

```
For nested model (how subjects differ day-to-day) -> singular, with only 3 days not enough to est.
* 30 subjects and 86 unique subject-day combinations
...
summary(DHEA.model)
tidy(DHEA.model, conf.int=T, exponentiate=T)
...
```{r}
# build DHEA model
DHEA.model2 <- lmer(log_dhea ~ t30_cap + t_after30_cap + 
                   (1|subjectid),
                 data=spit_df_filt)

summary(DHEA.model2)
tidy(DHEA.model2, conf.int=T, exponentiate=T)
```
...
newdat <- expand.grid(
  time_min = seq(0, 
                 max(spit_df_filt$bk_min_since_wk, na.rm = TRUE), by = 1)) %>%
  mutate(
...
pred_dhea <- newdat %>%
  mutate(
    fit = predict(DHEA.model, newdata = newdat, re.form = NA),
    hormone = "DHEA")

pred_cort <- newdat %>%
  mutate(
    fit = predict(cortisol.model, newdata = newdat, re.form = NA),
    hormone = "Cortisol")

pred_all <- bind_rows(pred_dhea, pred_cort)
...
```{r}
ggplot(pred_all, aes(x = time_min, y = fit)) +
  geom_line(linewidth = 0.8) +
  geom_vline(xintercept = knot, linetype = 2, color = "firebrick3") +
  facet_wrap(~ hormone, scales = "free_y", nrow=2) +
  #coord_cartesian(xlim = c(0, 800)) +
  labs(
    title = "Predicted mean log levels by minutes since waking",
    subtitle = paste0("Piecewise linear slopes with knot at ", knot, " minutes"),
    x = "Minutes since waking",
    y = "Predicted mean log concentration") +
