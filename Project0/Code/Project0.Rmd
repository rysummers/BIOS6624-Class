...
```{r}
# check residuals to determine if right skewness is extreme enough for this model
...
```{r}
# class recommened one - **debatable**
agrmnt.model2 <- lmer(
...
Intercept should be zero and slope should be 1. 

Interpretation:
In lme model of time differences (Booklet - Cap), agreement at waking did not show significant bias ($\beta_0:t=-1.1; p=0.29$); however, the +30-minute sample showed that participants recorded times approximately 16.3 minutes earlier (-4.1 + -12.2) than the Cap timestamp, on average. There is no evidence of disagreement for samples before lunch ($p=0.7$) and 600min after waking ($p=0.8$).
...
* High correlation does not preclude bias; the mixed model identifies a consistent offset at Sample 2 after accounting for subject-level variability.
...
tbl_regression(
  ...
  exponentiate = T,
  intercept = T,
  estimate_fun = label_style_sigfig(digits = 5),
  pvalue_fun = label_style_pvalue(digits = 3),
  label = list(
    t30 ~ "30min post wake",
    t_after30 ~ "After 30min")) %>%
  as_gt() %>%
  gt::tab_header(title = "Diurnal Patterns of Cortisol",
                 subtitle = "LMM Piece-wise Regression w/ Random Intercept")
```
...
tbl_regression(
  ...
  pvalue_fun = label_style_pvalue(digits = 3),
    ...
    t_after30 ~ "After 30min")) %>%
  as_gt() %>%
  gt::tab_header(title = "Diurnal Patterns of DHEA",
                 subtitle = "LMM Piece-wise Regression w/ Random Intercept")
```
...
ggplot(newdat, aes(x = time_min, y = fit)) +
  ...
  annotate("text", x = knot, y = max(newdat$fit, na.rm = TRUE),
           ...
           size = 4, color = "black") +
  labs(
    title = "Mean log(Cortisol) by minutes since waking",
